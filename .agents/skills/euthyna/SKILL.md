...
`--verify` 与 PoC 会在**用户的机器**上执行命令。跑任何复现/验证命令前：

- **先取得用户明确批准**，把要跑的命令原文列出来——不跑没报备的命令
- 能在沙箱/临时容器里跑的（如 Docker），**优先沙箱**；无法沙箱时说明风险再跑
- 报告里的 PoC 是给「拿到报告的人」复现用的；裁定过程中 agent 自己执行的每条命令同样适用本节
