...
| `test/` | 78 tests via `node --test`, no third-party framework. **The history tests build real git repositories rather than mocking**; the coverage tests carry fixtures shaped like both c8 and coverage.py output; the contract tests carry the shell-quoting and render-boundary regressions from PR #1 |
