...
3. **裁定者自己的会话也会被自动摘要**——这是 run 3 收尾时的新发现。四个补跑案例
   各自长出一条带结论的记忆条目，而它们的回传消息严格只有一行 `DONE`。
   也就是说 file-only 协议保住的是**主对话通道**；子代理会话本身就是第三个泄漏源，
   每次裁定完成后都要再扫一次记忆库。
...
## 事后跟进：case-08 相邻观察已裁定

第二轮 run 1 的裁定者在否掉 case-08 的命令注入断言时，另记了一条「输出路径可穿越」的
观察。该观察后来完成了独立的阶段 C 裁定：**TRUE POSITIVE**——写入侧路径穿越
（`archive.js:18` 的输出路径拼接无归属检查），六门禁全过，真实 bsdtar 3.8.8 实测：
目录外创建、既有文件被覆盖、tar 失败路径下输出文件仍被截断、负样本与「单个 `..` 不穿越」
的精确化俱全。两点含义：

1. **「假案例」≠「干净代码」**。case-08 的命令注入断言判假阳性是正确的，同一份源码里
   同时存在另一条真漏洞。裁定对象是断言，不是代码库。这一原则已写入第三轮设计
   （`bench/DESIGN-round3.md`）。
2. 该观察直接成为第三轮新真案例 p5 的来源；裁定中还实测到读取侧行为（归档收录了
   `data/` 之外的文件），是候选 p6 的证据。

裁定报告：`.scratch/adjacent-observations/p3b-archive-output-path/adjudication.md`
（gitignored，结论以本节为准）。
